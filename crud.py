...
def get_all_fruits(conn: Connection) -> List[Tuple]:
    ...
    """
    cursor = conn.cursor()
    cursor.execute("SELECT id, name, price, quantity, category FROM fruits ORDER BY name")
...
def get_fruit_by_id(conn: Connection, fruit_id: int) -> Optional[Tuple]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, name, price, quantity, category FROM fruits WHERE id = ?", 
        (fruit_id,)
    )
    return cursor.fetchone()


def add_fruit(conn: Connection, name: str, price: float, quantity: int, category: str) -> int:
    """
...
def update_fruit(conn: Connection, fruit_id: int, name: str, price: float, quantity: int, category: str) -> bool:
    """
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE fruits SET name = ?, price = ?, quantity = ?, category = ? WHERE id = ?",
        (name, price, quantity, category, fruit_id)
    )
    conn.commit()
...
def search_fruits(conn: Connection, search_term: str) -> List[Tuple]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, name, price, quantity, category FROM fruits WHERE name LIKE ? OR category LIKE ? ORDER BY name",
...
def get_fruits_by_category(conn: Connection, category: str) -> List[Tuple]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, name, price, quantity, category FROM fruits WHERE category = ? ORDER BY name",
        (category,)
    )
    return cursor.fetchall()
...
def get_low_stock_fruits(conn: Connection, threshold: int = 10) -> List[Tuple]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, name, price, quantity, category FROM fruits WHERE quantity < ? ORDER BY quantity",
        (threshold,)
    )
    return cursor.fetchall()
